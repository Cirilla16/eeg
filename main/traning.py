...
class TrainModel():
    def __init__(self,):
        self.base_path = '/Users/cirilla/Documents/Code/ml/eeg/files copy'
    ...
    def plot_test_metrics(self,test_metrics: Dict[str, float]):
    ...
    def train_model(self,epochs, batch_size, model, train_loader, val_loader, patience=5):
        ...
        for epoch in range(epochs):
            train_loss, train_acc = self.train(model, train_loader, optimizer, criterion, device)
            val_loss, val_acc, val_mae, val_mse, val_rmse = self.evaluate(model, val_loader, criterion, device)
            ...
            print(
                f"  Val   Loss: {val_loss:.4f} | Val   Acc: {val_acc:.4f} | MAE: {val_mae:.4f}| MSE: {val_mse:.4f} | RMSE: {val_rmse:.4f}")
    ...
    def test_model(self,model, test_loader, criterion, device):
        test_loss, test_acc, test_mae, test_mse, test_rmse = self.evaluate(model, test_loader, criterion, device)
    ...
    def get_loaders(self,epoch: int = 32, batch_size: int = 64, sfreq: int = 128) -> Tuple[DataLoader, DataLoader, DataLoader]:

        dataset = EEGDataset(data_dir=self.base_path, sfreq=sfreq, duration=1.0)
...
class TrainEEGNetModel(TrainModel):
    def run(self):
        sfreq = 128
        model = EEGNet(num_classes=3, sfreq=sfreq).to(device)
        epochs = 32
        batch_size = 64
        train_loader, val_loader, test_loader = self.get_loaders(epoch=epochs, batch_size=batch_size, sfreq=sfreq)
...
class TrainCSTMModel(TrainModel):
    def run(self):
        sfreq = 128
...
if __name__ == '__main__':
    trainEEG=TrainEEGNetModel()
    trainCSTM=TrainCSTMModel()
    trainEEG.run()
    trainCSTM.run()